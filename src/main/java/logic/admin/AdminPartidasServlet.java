...
public class AdminPartidasServlet extends HttpServlet {
    ...
    private List<Partida> getAllPartidas() {
        ...
        try (Connection conn = DbConn.getInstancia().getConn();
            ...
            while (rs.next()) {
                count++;
                System.out.println("[AdminPartidasServlet] Procesando partida " + count + ", ID: " + rs.getInt("id"));
                ...
                // Obtener username e historia título manualmente
                p.setUsuarioUsername(getUsernameById(rs.getInt("usuario_id")));
                p.setHistoriaTitulo(getHistoriaTituloById(rs.getInt("historia_id")));
